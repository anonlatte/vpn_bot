...
def process_message(message):
    ...
    if text == "/start":
        logger.info(
            "Команда /start от пользователя %s", chat_id, extra={"username": username}
        )
        welcome_message = (
        ...
        logger.info(
            "Команда /help от пользователя %s", chat_id, extra={"username": username}
        )
        core.send_message(
            chat_id,
        ...
        logger.info(
            "Получен контакт от пользователя %s", chat_id, extra={"username": username}
        )
        # Пользователь отправил контакт
        phone_number = contact.get("phone_number", "")
        if str(contact["user_id"]) != str(chat_id):
            logger.warning(
                "Пользователь %s отправил контакт другого пользователя",
                chat_id,
        ...
        logger.info(
            "Получено текстовое сообщение от пользователя %s: %s",
            chat_id,
            text,
            extra={"username": username},
        )
...
def handle_callback_query(callback_query):
    ...
    if data == "get":
        ...
        logger.info(
            "Пользователь %s запросил помощь для платформы %s", chat_id, platform
        )
