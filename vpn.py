...
def create_vpn_account(chat_id, full_name, phone_number):
    ...
    expired_chat_ids = [
        cid
        for cid, data in cfg.user_data.items()
    ...
    if chat_id in cfg.user_data:
        ...
        if now - last_request_time < timedelta(minutes=2):
            ...
            del cfg.user_data[
                chat_id
            ]  # Удаляем пользователя из cfg.user_data, так как таймаут истек
    ...
    try:
        ...
        if login_result.get("success"):
            ...
            core.send_message(
                chat_id, "Не удалось войти в API 3xui. Проверьте логин и пароль."
            )
            return
    ...
    try:
        ...
        if inbounds_data.get("success"):
            ...
            if not inbounds:
                ...
                core.send_message(
                    chat_id, "Нет доступных inbounds для добавления пользователя."
                )
                return
            ...
            logger.error(
                "Не удалось получить список inbounds: %s", inbounds_data.get("msg")
            )
    ...
    try:
        ...
        if inbound_details_data.get("success"):
            ...
            logger.error(
                "Не удалось получить детали inbound: %s",
                inbound_details_data.get("msg"),
            )
    ...
    if client_exists:
        ...
        core.send_photo(
            chat_id,
            bio,
            caption=f"Ваш VPN настроен. Сканируйте QR-код или используйте ссылку ниже для настройки клиента.\n{hidden_vless_link}",
        )
        ...
        try:
            ...
            add_client_response = session.post(
                add_client_url, json=payload, headers=headers
            )
            ...
            if add_client_data.get("success"):
                ...
                logger.error(
                    "Не удалось добавить клиента: %s", add_client_data.get("msg")
                )
                core.send_message(
                    chat_id,
                    f'Не удалось добавить клиента: {add_client_data.get("msg")}',
                )
        except requests.exceptions.RequestException as e:
